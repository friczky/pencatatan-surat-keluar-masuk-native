...
if (isset($_POST['tambah'])) {
        ...
        $file_tmp = $_FILES['file']['tmp_name'];
            if (in_array($ekstensi, $ekstensi_diperbolehkan) === true){
                ...
                if ($simpan) {
                    $sql = "INSERT INTO arsip_surat (id_format_surat,jenis_file,file) VALUES ('$id_format_surat',1,'$file_save')";
                    $simpan=mysqli_query($koneksi,$sql);
                    if ($simpan) {
                        echo '<script>alert("Berhasil menambahkan surat");window.location.href="index.php";</script>';
                    }
                }
                else {
                    echo '<script>alert("Gagal menambahkan surat");window.location.href="index.php";</script>';
                }
            }
            else{
